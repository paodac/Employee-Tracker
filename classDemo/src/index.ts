...
async function addRole(){
    ...
    const departments= data.rows.map((d:any)=>({name: d.name, value: d.id}))
    const response= await inquirer.prompt([
        {
            type:'input',
            name:'role_title',
...
async function addEmployee(){
    ...
    const roles= roleData.rows.map((d:any)=>({name: d.title, value: d.id}))
    const employeeData= await pool.query ('SELECT * FROM employee')
    const employees= employeeData.rows.map((d:any)=>({name: d.first_name + ' '+ d.last_name, value: d.id}))
